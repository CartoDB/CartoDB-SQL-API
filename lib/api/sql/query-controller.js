...
const { sendPubSubMetric } = require('../middlewares/pubsub-metrics');

const { RATE_LIMIT_ENDPOINTS_GROUPS } = rateLimits;
const PSQL = require('cartodb-psql');

module.exports = class QueryController {
    constructor (metadataBackend, userDatabaseService, statsdClient, userLimitsService, pubSubMetricsService) {
    ...
    route (sqlRouter) {
        ...
        const queryMiddlewares = () => {
            return [
                ...
                formatter(),
                content(),
                handleQuery({ stats: this.stats, pubSubMetricsService: this.pubSubMetricsService })
            ];
        };
...
function handleQuery ({ stats, pubSubMetricsService } = {}) {
        ...
        try {
            ...
            if (req.profiler) {
                ...
                opts.beforeSink = function () {
                    ...
                    sendPubSubMetric(pubSubMetricsService, req, res);
                };
            }
